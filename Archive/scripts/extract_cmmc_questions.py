...
def clean_explanation(text: str) -> str:
    ...
    # Collapse multiple newlines into paragraph breaks and extra spaces
    text = re.sub(r'\n{3,}', '\n\n', text)  # Convert 3+ newlines to 2 newlines
...
def parse_questions(text: str) -> list[dict]:
    ...
    for idx, m in enumerate(matches):
        ...
        b_match = re.search(r'\nB\.\s*(.*?)(?=\nC\.\s*|$)', content, re.DOTALL)
        c_match = re.search(r'\nC\.\s*(.*?)(?=\nD\.\s*|$)', content, re.DOTALL)
        d_match = re.search(r'\nD\.\s*(.*?)(?=\s*Answer:)', content, re.DOTALL)  # Stop before Answer
